...
description: "Write or improve KDoc for public APIs in support-markdown. Use for Dokka updates, plugin class docs, controller docs, listener docs, span docs, and explaining how downstream apps should use or configure the library."
...
4. For plugin classes, explain which markdown element is handled and how the plugin slots into the `Markwon` builder.
...
7. Link adjacent types with KDoc references so Dokka helps consumers move through the API surface.
8. If the type belongs to a new plugin package, consider whether `CorePlugin` or `IMarkdownPlugin` docs also need an update.
